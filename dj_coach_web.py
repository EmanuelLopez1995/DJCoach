...
def deck_card(deck: dict[str, Any], name: str, accent: str) -> str:
    ...
    <section class="deck-card" style="--accent:{accent}">
      <header><span>TRACK DECK</span><h2>DECK {name}</h2></header>
...
def render_dashboard(snapshot: dict[str, Any]) -> str:
    ...
    <main class="dashboard-shell">
      ...
      <div class="decks-layout">
        {deck_card(snapshot['deck_a'], 'A', '#36d7ff')}
        {deck_card(snapshot['deck_b'], 'B', '#ff4fd8')}
      </div>
...
@media (max-width:900px) { .decks-layout,.bottom-layout { grid-template-columns:1fr; }.state-grid { grid-template-columns:repeat(2,1fr); }.topbar { align-items:flex-start; }.connection { max-width:50%; }.debug-strip { grid-template-columns:1fr; }.debug-strip small { display:none; } }
"""
